...
exports.updateContentOnCourse = async (req, res) => {
  if (!req.body.isAdd)
    until.validateJson(courseSchema.updateContentOnCourse, req.body);
  else
    until.validateJson(new courseSchema.updateContentOnCourseisAdd(), req.body);
  const result = await CourseService.updateContentOnCourse(req.body);
  resDataModify(res, result);
};
exports.deleteContentCourse = async (req, res) => {
  if (!req.query.id)
    throw ErrorService.somethingWentWrong("You need 'id' in params query");
  const result = await CourseService.deleteContentCourse(req.query.id);
  resDataModify(res, result);
};
exports.addContent = async (req, res) => {
  until.validateJson(courseSchema.addContentSchema, req.body);
  const result = await CourseService.addContent(req.params.id, req.body);
  resDataModify(res, result);
};
