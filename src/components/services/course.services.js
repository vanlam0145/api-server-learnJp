...
exports.getById = async (id) =>
  await CourseModel.findById(id).populate('contents').lean();
exports.create = async function (body) {
  const { title, content, _id } = body;
  const contents = await untilServices.exec(ContentModel.create(content));
  const course = await untilServices.exec(
    CourseModel.create({ title, contents })
  );
...
exports.makeQuestion = async (id) => {
  ...
  if (!courses)
    throw ErrorService.somethingWentWrong("You don't have any couser");
...
exports.delete = async function (req) {
  const courses = await untilServices.exec(
    CourseModel.findByIdAndDelete({ _id: req.params.id })
  );
  await Promise.all([
    untilServices.exec(
      ContentModel.deleteMany({ _id: { $in: courses.contents } })
    ),
    UserModel.updateOne(
      { _id: req.user._id },
  ...
  if (Types.ObjectId.isValid(id)) {
    if (!contents || contents.length == 0)
      throw ErrorService.somethingWentWrong(
        "This request don't have any 'contens'"
      );
    else {
      for (let content of contents) {
        if (!Types.ObjectId.isValid(content))
          throw ErrorService.somethingWentWrong(
            'This request need contents is Array ObjectId'
          );
      }
      const couse = await untilServices.exec(
        CourseModel.updateOne(
          { _id: id },
          { $pull: { contents: { $in: contents } } }
        )
      );
      return await untilServices.exec(
        ContentModel.deleteMany({ _id: { $in: contents } })
      );
    }
  } else
    throw ErrorService.somethingWentWrong(
      'This request need course_id is ObjectId'
    );
};
...
exports.addContent = async (id, body) => {
  let contentNews = [];
  let idContentNews = [];
  body.forEach((element) => {
    ...
    contentNews.push(newContent);
    idContentNews.push(newContent._id);
  });
  try {
    await ContentModel.insertMany(contentNews);
    return await CourseModel.findByIdAndUpdate(
      id,
      {
        $push: { contents: idContentNews },
      },
      { new: true }
    );
  } catch (error) {
