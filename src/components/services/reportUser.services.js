...
exports.update = async (req) => {
    if (req.user.role == 'user') {
        const reportData = await ReportUserModel.findOne({ _id: req.params.id, accusersId: req.user._id })
        if (!reportData)
    ...
    if (["helpful", "harmful"].indexOf(req.body.confirm) != -1)
        req.body.status = 'processed'
    else {
        delete req.body.confirm
        req.body.status = 'process'
    }
    return await untilServices.exec(ReportUserModel.findOneAndUpdate(
        { _id: req.params.id },
        { $set: req.body },
        { new: true }))
}
